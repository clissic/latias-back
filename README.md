...
Documentación de los endpoints del backend de LATIAS Academia. El servidor expone la API bajo el prefijo `/api` y utiliza tokens Bearer (JWT) para autenticación en rutas protegidas. Incluye: usuarios y roles (Cadete, Instructor, Administrador, Gestor, checkin), cursos, eventos, barcos y flota, certificados, **solicitudes a gestor** (ship-requests), instructores, contacto, **códigos de descuento** (solo Administrador), Mercado Pago y subida de archivos.
...
17. [Archivos estáticos](#archivos-estáticos)
