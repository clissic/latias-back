...
**Progreso del proyecto: 52,5% completado**
- ✔️ Completados: 52 puntos (52%)
- 🟡 En desarrollo: 1 punto (0,5%)
- ❌ Pendientes: 47 puntos (0%)
...
Documentación de los endpoints del backend de LATIAS Academia. El servidor expone la API bajo el prefijo `/api` y utiliza tokens Bearer (JWT) para autenticación en rutas protegidas. Incluye: usuarios y roles (Cadete, Instructor, Administrador, Gestor, checkin), cursos, eventos, barcos y flota, certificados, **solicitudes a gestor** (ship-requests), instructores, contacto, **códigos de descuento** (solo Administrador), **wallet** (instructores y gestores), Mercado Pago y subida de archivos.
...
- **POST `/create`:** Body con `courseId`, `sku`, `courseName`, `price`, `category` requeridos; opcionales: `bannerUrl`, `image`, `shortImage`, `currency`, `shortDescription`, `longDescription`, `duration`, `difficulty`, `professor` (datos del **instructor**, ver modelo), `modules`, `selectedInstructorId`. Respuesta 201 con curso creado.
